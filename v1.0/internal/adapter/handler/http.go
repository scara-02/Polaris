...
func (h *HTTPHandler) GetRoute(c *gin.Context) {
	sLat, _ := strconv.ParseFloat(c.Query("src_lat"), 64)
	sLon, _ := strconv.ParseFloat(c.Query("src_lon"), 64)
	dLat, _ := strconv.ParseFloat(c.Query("dest_lat"), 64)
	dLon, _ := strconv.ParseFloat(c.Query("dest_lon"), 64)

	geometry, err := h.engine.GetDriverRoute(sLat, sLon, dLat, dLon)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to calculate physical route"})
		return
	}

	// Send the GeoJSON directly to the client
